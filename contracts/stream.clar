...
(define-read-only (balance-of
  ...
  (let (
    ...
    (block-delta (calculate-block-delta (get timeframe stream) (get paused-at stream)))  ;; Pass paused-at
...
;; Calculate the number of blocks a stream has been active, accounting for pauses
(define-read-only (calculate-block-delta
    (timeframe (tuple (start-block uint) (stop-block uint)))
    (paused-at uint)  ;; New parameter: pass from stream
  )
  (let (
    (start-block (get start-block timeframe))
    (stop-block (get stop-block timeframe))
    (current-height (if (> paused-at u0) paused-at stacks-block-height))  ;; Use paused-at if set
...
(define-public (pause-stream
  ...
  (let (
    ...
    (asserts! (is-eq contract-caller (get sender stream)) ERR_UNAUTHORIZED)
    (asserts! (is-eq (get paused-at stream) u0) ERR_STREAM_ALREADY_PAUSED)
    (map-set streams stream-id 
      (merge stream {paused-at: stacks-block-height})
    )
    (ok stacks-block-height)
  )
)

;; Resume a paused stream (resumes payment)
(define-public (resume-stream
  ...
  (let (
    ...
    (asserts! (is-eq contract-caller (get sender stream)) ERR_UNAUTHORIZED)
    (asserts! (> (get paused-at stream) u0) ERR_STREAM_NOT_PAUSED)
    (map-set streams stream-id 
      (merge stream {paused-at: u0})
    )
    (ok true)
  )
)
...
(define-public (cancel-stream (stream-id uint))
  (let (
    (stream (unwrap! (map-get? streams stream-id) ERR_INVALID_STREAM_ID))
    (sender-balance (balance-of stream-id (get sender stream)))
    (recipient-balance (balance-of stream-id (get recipient stream)))
  )
    (asserts! (or (is-eq contract-caller (get sender stream)) (is-eq contract-caller (get recipient stream))) ERR_UNAUTHORIZED)
    (asserts! (> (get stop-block (get timeframe stream)) stacks-block-height) ERR_STREAM_STILL_ACTIVE)  ;; Only if not ended
    (try! (as-contract (stx-transfer? sender-balance tx-sender (get sender stream))))
    (try! (as-contract (stx-transfer? recipient-balance tx-sender (get recipient stream))))
    (map-delete streams stream-id)  ;; Remove the stream
    (ok true)
  )
)
